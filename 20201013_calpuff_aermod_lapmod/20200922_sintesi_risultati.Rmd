...
Con l'obiettivo di confrontare tramite una "metrica comune" i risultati di run modellistici, che spesso risultano difficilmente comparabili, sia nel tempo che nello spazio, viene di seguito proposto il calcolo della "distanza di separazione", cioè la distanza che individua le isoplete di concentrazione ambientale fissate da un predeterminato livello soglia ad un specifica probabilità (nel caso specifico, il 98° percentile delle concentrazioni ambientali pari ai livelli di 1, 3, 5 uoE/m3).
...
Il grafico seguente rappresenta la distanza stimata da ciascuna configurazione modellistica tra il punto di sorgente (centroide) e i punti individuati lungo le isoplete che delimitano le concentrazioni di 1, 3, 5 uoE/m3 relative al 98° percentile orario calcolata ad una prefissata 
coordinata orizzontale (azimut) rispetto al Nord (bearing angle), con un passo di 5 gradi di ampiezza (da 0 a 360).
...
Sembra quindi emergere, in termini più generali, la spiccata attitudine di CALPUFF quale modello di "mesoscala", la significativa differenza nei risultati di AERMOD in funzione dell'algoritmo utilizzato per la caratterizzazione della sorgente (area vs. volume) e, per finire, la caratteristica tendenza di LAPMOD a "lavorare su distanze più brevi", con una maggiore propensione per la valutazione nel "campo vicino".
...
E' comunque evidente che la 'forma' delle isoplete risulta significativamente condizionata dalla direzione e velocità del vento che viene rappresentata nel grafico seguente riferito al punto corrispondente al centro del dominio di calcolo.
