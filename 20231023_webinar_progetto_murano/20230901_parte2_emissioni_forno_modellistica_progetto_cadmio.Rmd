...
---
## Scenari di valutazione: sintesi 
...
- .red[**Scenario 1 vs. Scenario 2: 'effetto filtro'**]
  ...
  - 'caso peggiore' con alimentazione a .red[*cotisso* senza obbligo di sistemi di filtrazione] *vs.* 'caso peggiore' con forno alimentato a .red[miscela minerale di *CdS* e sistema di filtrazione]; 
  .red[*nota bene*]: valuta effetti ambientali di una .red[efficienza di abbattimento] del sistema di filtrazione pari ad un .red[fattore 30] (approccio cautelativo perchè reale efficienza di abbattimento è circa 3 ordini grandezza, cfr. misure monte vs. valle!);
...
*senza filtro (Scenario 1) vs. con filtro (Scenario 2)*
...
source('./rscript_source/03_read_grid_plot_sc1_vs_sc2_f3_presentazione.R')

include_graphics("./img_pres/contour_sc1_vs_sc2_avg.png")

```

.tiny[.red['+' recettore, 'o' sorgente]; nota bene: per i due scenari lo stesso colore delle isolinee non rappresenta lo stesso valore di concentrazione (cfr. legenda)]

.red[&rarr;] [*zoom img*](./img_pres/contour_sc1_vs_sc2_avg.png)

---

## Scenario 2 vs. Scenario 3: 'effetto direzione' (tabelle)

*camino verticale (Scenario 2) vs. camino orizzontale (Scenario 3)*

.red[
- max dominio: camino orizzontale +20% per [max 24h] e per [98 pct 24h], +16% per [media anno];

- max ai recettori: incremento maggiore presso Foscolo (più vicino!), +9% per [max 24h], +16% per [98 pct 24h], +10% per [media anno];
]
...
source('./rscript_source/03_read_grid_plot_sc2_vs_sc3_f3_presentazione.R')

include_graphics("./img_pres/contour_sc2_vs_sc3_avg.png")

```

.tiny[.red['+' recettore, 'o' sorgente]; nota bene: per i due scenari lo stesso colore delle isolinee non rappresenta lo stesso valore di concentrazione (cfr. legenda)]

.red[&rarr;] [*zoom img*](./img_pres/contour_sc2_vs_sc3_avg.png)

---
## Scenario 2 vs. Scenario 4: 'effetto diluizione' (tabelle)

*camino 6 m (Scenario 2) vs. camino 12 m (Scenario 4)*
...
source('./rscript_source/03_read_grid_plot_sc2_vs_sc4_f3_presentazione.R')

include_graphics("./img_pres/contour_sc2_vs_sc4_avg.png")

```

.tiny[.red['+' recettore, 'o' sorgente]; nota bene: per i due scenari lo stesso colore delle isolinee non rappresenta lo stesso valore di concentrazione (cfr. legenda)]

.red[&rarr;] [*zoom img*](./img_pres/contour_sc2_vs_sc4_avg.png)
...
---
## Valutazione finale dei risultati scenari di contrasto

- l'impiego di sistemi di filtrazione è di fondamentale importanza: a parità di tutti gli altri parametri, i valori di concentrazione stimati al suolo dal sistema modellistico si riducono di decine di volte; 

- la possibilità di installare adeguati sistemi di filtrazione deve essere valutata, oltre che dal punto di vista economico, anche in funzione dei locali produttivi (spesso con spazi molto spesso limitati), che nel contesto insediativo dell’isola potrebbe risultare un fattore limitante o quanto meno di significativo impedimento operativo;

- a parità di rateo emissivo e di quota di emissione, i valori di concentrazione
stimati al suolo sono minori per il camino con sbocco verticale
rispetto a quello orizzontale;

- a parità di rateo emissivo e di diametro del camino di emissione, i valori di
concentrazione al suolo sono notevolmente inferiori al raddoppio dell'altezza del camino (da 6 m a 12 m); considerato il contesto urbanistico ed architettonico dell’isola è una soluzione che, anche in questo caso, potrebbe non essere sempre facilmente realizzabile.
