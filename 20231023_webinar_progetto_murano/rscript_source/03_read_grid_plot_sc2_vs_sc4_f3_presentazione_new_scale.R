...
breaks <- c(0.002, 0.005, 0.01, 0.02, 0.05, 0.1)
...
# define title
title <- "Scenario 2 (6 m) - media anno" 
...
# define title
title <- " Scenario 4 (12 m) - media anno" 
...
lp_avg %>%
  ...
  theme(panel.grid.major = element_line(linetype = 3),
        legend.position = "bottom",
        axis.text.x = element_text(angle = 90, vjust = 0.5, size=8),
        axis.text.y = element_text(vjust = 0.5, size=8)) -> sc4_avg

#########################################################################################################

# plot

sc2_avg | sc4_avg + plot_layout(nrow = 1, widths = 1, heights = 1)

ggsave('contour_sc2_vs_sc4_avg_new_scale.png', path='./img_pres/', width = 10, height = 5)
