## export imgs for presentation

## ---- read-data-grid-f3-sc4 ----
...
fl <- './data_input/grd_f3_sc_4/Average_Period.grd '

# lapost from raster to dataframe 
lp_avg <- raster(fl) %>%
...
# define title
title <- " Scenario 4 - media annuale" 
...
lp_avg %>%
  ...
  geom_text_contour(aes(x = x, y = y, z = z ), 
                    ...
                    #label.placer = label_placer_flattest(rot_adjuster = isoband::angle_fixed(60))
                    ) +
...
ggsave('contour_sc4_avg.png', path='./img_pres/', width = 5, height = 5)

## ---- ggplot-f3-sc4-p98 ----

# file p98 avg 24h
fl <- './data_input/grd_f3_sc_4/Percentile_24h.grd'
...
ggsave('contour_sc4_p98.png', path='./img_pres/', width = 5, height = 5)

## ---- ggplot-f3-sc4-max ----

# file max avg 24h
fl <- './data_input/grd_f3_sc_4/Maximum_24h.grd'
...
ylim <- c(ll[2], ll[2]+2250)

# define breaks
breaks <- c(0.05, 0.1, 0.2, 0.5, 1)

# define title
title <- " Scenario 4 - max 24h" 
...
ggsave('contour_sc4_max.png', path='./img_pres/', width = 5, height = 5)
