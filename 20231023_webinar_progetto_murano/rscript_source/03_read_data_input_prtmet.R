...
col_names <- read_table('./data_input/prtmet_2017.dat', skip= 1, n_max = 0) %>% names() %>% tolower()

# remove empty column
col_names <- head(col_names, -1)

met<-read_table('./data_input/prtmet_2017.dat', 
                col_names= col_names, 
                skip=4, 
