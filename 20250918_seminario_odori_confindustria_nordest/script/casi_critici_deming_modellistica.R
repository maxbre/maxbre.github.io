...
## PDCA: il possibile ruolo della modellistica
...
dm <- d %>%
  ...
  set_node_position(node=5, x=1.2*k, y=0*k) %>% 
  
  add_edge(from=2, to=5, edge_aes = edge_aes(color ="red", 
                                             style="dashed"
                                             #, label = "  ex ante"
  ))%>%
  
  add_edge(from=5, to=3, edge_aes = edge_aes(color ="red", 
                                             style="dashed"
                                             #, label = "ex ante"
  ))%>%
  ...
  set_node_position(node=6, x=-0.2*k, y=1*k) %>% 
