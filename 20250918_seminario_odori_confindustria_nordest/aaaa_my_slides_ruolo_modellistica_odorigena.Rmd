---
title: '<br><br>Modellistica di impatto odorigeno'
...
institute: "ARPAV - Dipartimento Regionale Qualità dell'Ambiente<br>Unità Organizzativa Qualità dell'Aria"
date: "Marghera, 18/09/2025<br><br><br>.right[.small[.lightgrey[last updated: `r Sys.Date()`]]]"
output:
...
- #### .red[Molestia (Nuisance)]
  ...
  .red[effetto cumulativo] prodotto da ripetuti eventi di disturbo in un lungo periodo di tempo, che genera un .red[comportamento modificato o alterato] nel recettore;
...
<br>
**La simulazione modellistica fornisce una risposta utile?<br>Ed eventualmente in quali termini?**
...
# Quali sorgenti con più frequente criticità?
...
Misura la .red[frequenza di accadimento degli eventi odorigeni] presso i recettori che dipende dalla variazione temporale delle emissioni e dalle condizioni meteorologiche e dispersive dell'atmosfera.
...
- stima output concentrazioni odorigene (peak-to-mean ratio)
...
- trattazione delle stime di concentrazione odorigena ai recettori come .red[percentile "alto"] (98° percentile medie 1h, in Italia!).
...
- aspetto non esplicitamente trattato nelle simulazioni (in Italia): difficile da "chiudere" in modo rigoroso ed obiettivo.
...
Il .red[luogo] dove si verifica il disturbo o molestia olfattiva presuppone una .red[sensitività] differente in funzione:

- della destinazione d'uso del territorio, del contesto ambientale, economico (*pecunia non olet*) e sociale (accettabilità);
...
- destinazione d'uso (classificazione urbanistica) del territorio
...
<br>
.red_bold[Quali fattori FIDO* sono risolti con la simulazione modellistica?]
...
.small[Ricomposizione FIDO secondo i contenuti tecnici degli "Indirizzi " (ex MASE n. 309/2023)]
...
- .red[Intensity]: non viene valutato, è considerato solo "indirettamente" tramite *proxy*  .red[concentrazione odorigena] (diverso da intensità!).
...
- L'olfattometria dinamica (UNI EN 13725) è la tecnica di riferimento per la quantificazione delle .red[*concentrazioni*] di odore .red[*alla sorgente (emissioni)*], ma .red[non adeguata] per valutare l’impatto ai recettori. 
<br>
...
- La principale limitazione della modellistica risiede nella difficoltà teorica e pratica (non ancora superata) di ricostruire le .red[fluttuazioni di concentrazione], cioè i .red[picchi di odore su pochi secondi].
<br>
...
- L'approccio più utilizzato (semplice e pragmatico) è l'applicazione del fattore moltiplicativo "semi-empirico" .red[*Peak-to-Mean Ratio*] alle stime delle medie 1h: ma è una soluzione non del tutto rigorosa!
...
.small[
espressa in .red[unità odorimetriche europee] $[OU_E~m^{-3}]$ rappresenta il .red[numero di diluizioni] di aria neutra necessarie per portare la concentrazione del campione alla .red[soglia di rilevamento dell'odore] da parte del panel di esaminatori; assunzione forte, di perfetta trasferibilità: la risposta del panel all'odorante di riferimento (n-butanolo) è uguale alla soglia di percezione per qualsiasi altro tipo di odorante
]
...
knitr::include_graphics("./img_pres/flowchart/c_oer/c_od_no_label_color.png")
...
knitr::include_graphics("./img_pres/flowchart/c_oer/oer_no_label_color.png")
```
...
#### .red[significative (da considerare)]

$OER$ > 500 $UO_E~s^{-1}$

#### .red[non significative (possono non essere considerate)]

$C_{OD}$ < 80  $UO_E~m^{-3}$ 

indipendentemente dalla portata volumetrica emessa
...
Le valutazioni modellistiche stimano i .red[**percentili** della concentrazione odorigena di picco], su base annuale, da confrontare con un .red["valore soglia"] definito dalla normativa.
...
Questi valori numerici, secondo un'ipotesi di trasferibilità (forte!), sono .red[convenzionalmente assunti in aria ambiente per valutare l'impatto odorigeno presso i  recettori].
...
per cui viene "concesso" che i recettori possano percepire l'odore:

- .red[**al massimo per 175 ore/anno (corrisponde al 2% di 8760 ore).**]
...
# Quali applicazioni ARPA per la modellistica?<br>E con quali strumenti?
<br>
## .white[Questionario RRTEM05 SNPA 2023]
...
# Valutazione su differenti livelli

#### .red[Rispetto a una o più sorgenti su "area limitata":]
...
#### .red[Rispetto a più sorgenti su "area estesa":]

- .red[*valutazioni "strategiche" (zonizzazione)*]  
<br>.small[
   - sistema di supporto decisionale alla pianificazione territoriale

Allo stato attuale la "valutazione strategica" è in gran parte inespressa.<br>Prevista indirettamente dagli "Indirizzi" è ricollegabile alla definizione dei .red[criteri di accettabilità] ai recettori che sono  basati su principi di .red[pianificazione urbanistica e territoriale].

]

---

# Valutazioni "strategiche" .small[(1/3)]
...
# Valutazioni "strategiche" .small[(2/3)]

### Legato (...o meglio, un auspicio!)

$\Rightarrow ~$ ex "Indirizzi" MASE, 2023:

*"Appare utile [...] che le competenti autorità territoriali possano assicurare, a livello locale, un .red[efficace coordinamento con le norme e procedure relative alle emissioni acustiche e quelle relative alla pianificazione territoriale/urbanistica"*] (

.small[
Definisce un approccio .red[*strategico (preventivo)*] nella valutazione di impatto odorigeno attraverso il coordinamento e la verifica di compatibilità con gli strumenti sovraordinati di pianificazione urbanistica e territoriale.
]

I contenuti tecnici che definiscono i criteri di accettabilità odorigena richiamano quelli tipici della .red[*"Zonizzazione Acustica"*].

<br>
$\Rightarrow ~$.red[**Domanda aperta**:]
...
- Non esistono "LG ufficiali" sui contenuti tecnici di una valutazione modellistica odorigena "strategica" per una classificazione "osmogena" del territorio.

- .red["Modellistica inversa"?] Stima del .red["termine sorgente"]: per ogni cella del dominio di calcolo  viene stimata .red[l’emissione massima di odore [OUE/s] "consentita"] per il rispetto dei valori di accettabilità presso i recettori.
...
- Necessità di concertazione su modi e contenuti tecnici con autorità competenti a livello locale, regionale e nazionale per "spostare" una parte delle valutazioni a livello di pianificazione.
...
Il processo decisionale di autorizzazione e valutazione dell’impatto odorigeno degli “Indirizzi" come un .red[**ciclo di miglioramento continuo (Deming, PDCA)**].
...
.red[**Quanto e come cambiano le stime modellistiche di impatto odorigeno con differenti (alternative) modalità di calcolo PMR?**]<br>(oltre al "classico" PMR = 2.3, usato in Italia!)
...
- distanza di separazione: PMR dinamico >  PMR statico;

- impatto recettori  < 150 m da sorgente: PMR dinamico +1 ordine di grandezza.

.red[**di tipo regolatorio (su iter autorizzativo)**]

- necessaria forte *consapevolezza tecnica* sulle configurazioni modellistiche *adottate (o non adottate)* dai proponenti e *richieste (o non richieste)* da autorità di controllo;
