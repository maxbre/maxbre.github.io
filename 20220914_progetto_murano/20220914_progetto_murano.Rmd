...
institute: "</br>ARPAV: <sup>(#)</sup>DAPVE, <sup>(##)</sup>DQA"
...
##  Murano, Fondamenta Colleoni c/o Scuola Foscolo
...
tab_2021<- hm %>%
  ...
  summarise(As=format(round(mean(As),1), nsmall=1),
            ...
            Pb=format(round(mean(Pb),1), nsmall=1)
            )

knitr::kable(tab_2021[2:5], format = "html", caption = "c/o Scuola Foscolo", digits=1, align = 'c')
...
<br/>
.red[tra gli inquinanti normati le concentrazioni medie annuali di Cadmio rappresentano una significativa criticità ambientale]
...
hm %>%
  ...
  geom_boxplot() +
  stat_summary(fun = "mean", color="red", shape=18, size=1)+
  coord_trans(y="log10")+
...
hm %>%
  ...
  geom_hline(yintercept = 0.1, linetype="dashed", color = "green", size=0.5)+
  annotate(geom="text", x=ymd("2016-10-01"), y=0.08, label="LQ", colour="green", size=3.5)+
...
---
### Arsenico, medie 24h 2016-2021, Murano Foscolo
...
#### run con dati storici su una singola sorgente 'rappresentativa'
