...
è possibile fare un piccolo calcolo spannometrico di hmix, per quella determinata ora e per quel determinato giorno 
...
prima valutazione speditiva di tipo 'esplorativo', approccio 'obs - no obs'
...
- la quota di base della sorgente determina la differenza di elevazione tra sorgente e recettori e l'elevazione del pennacchio rispetto al terreno (*cfr. 'AERMOD implementation guide', p. 4.5*)

- .red[stima con elevazione di base della sorgente (90 m)] 

cfr. file: [02_aermod_base90m.html](./02_aermod_base90m.html)  
(stima che evidenzia però altri 'problemi', trattati in slides successive)
...
.small[NB: eiste opzione .red[ADJ_U*] (friction velocity) per aggiustamento rispetto a calme di vento (in condizioni ELEV) ma ovviamente entro certi limiti, non per questo caso...] 
---
...
## 'punto calmet', varie configurazioni
##.red[(segue files mappe)]
