...
background-image: url(img_css/logo_arpetta.jpeg)
...
una di quelle giornate in cui in pianura si sguazza nell'aerosol vista da
...
è possibile fare un calcolo a spanne hmix, per quella determinata ora e per quel determinato giorno 
...
### periodo di riferimento: 01/10/17 – 31/03/18

## cfr. file: [01_calpuff.html](./01_calpuff.html)

[(jump to lapmod)](#lapmod)
---
layout: true
...
```{r calmet-windrose-month, fig.width=10, fig.height=7, fig.align='left',  echo=FALSE}
...
- cmt castelfranco v. (10 m): temp, vv, dv, prec, srad, rhu; lami: tcc, press

- radiosondaggio: san pietro capofiume 16144

- risoluzione griglia calcolo: 250 m, elevated terrain ('receptors.rec'), orografia DTM regione Veneto (geotiff): risoluzione x, y = 30 m
...
- .red[NO elevazione base sorgente, optional parameter, default = 0, nel caso ELEV è un errore !! (trattato in slides seguenti, qui focus calme di vento)]

approccio 'corretto' è inserire l'elevazione di base della sorgente, cfr. file: [02_aermod_base90m.html](./02_aermod_base90m.html) (ma con altri 'problemi' trattati in slides seguenti)
...
```{r aermet-cmt-windrose-month, fig.width=10, fig.height=7, fig.align='left',  echo=FALSE}
...
.small[NB: eiste opzione .red[ADJ_U*] (friction velocity) per aggiustamento rispetto a calme di vento (in condizioni ELEV) ma ovviamente entro certi limiti...] 
---
## AERMET - hmix (cmt - castelfranco v.)

NB: molti NA + alcuni 'spikes' a 4000 m (fuori scala, dic 2017 e feb 2018)

```{r aermet-cmt-hmix, fig.width=10, fig.height=7, fig.align='left',  echo=FALSE, warning=FALSE}
...
- in varie configurazioni rispetto all'elevazione base sorgente:
  ...
  - default 0, optional parameter .red[(!!! attenzione, errore  con ELEV)]
...
```{r aermet-lami-windrose-month, fig.width=10, fig.height=7, fig.align='left',  echo=FALSE}
...
terrain impacting (horizontal plume) and terrain-following plume<sup>*<sup/>  

```{r  aermod-plume, echo=FALSE, out.width = '70%', out.height='70%', fig.align='center'}
...
$Z_p$
.red[**the 'flagpole receptor height', i.e. the height above terrain where concentration is calculated according to the terrain following state**]
...
$H_c$ can be thought of as the level in the stable atmosphere where the flow has sufficient kinetic energy to overcome the stratification and rise to the height of terrain.
...
[(hop back to roughness)](#roughness)
]
...
- confrontare i risultati rispetto a differenti approcci modellistici (‘plus’ molto ‘dispendioso’ e non sempre fattibile)
...
.footnote[.left[[(hop back to calpuff)](#calpuff)]]
