...
ma rispetto alla precedente configurazione cambia  
...
**to be noted that for any concentration calculation all heights $(Z)$ are referred to stack base elevation $(Z_t)$** 
...
In **stable** conditions, the horizontal plume *'dominates'* and is given greater weight while in **neutral** and **unstable** conditions, the plume travelling over the terran is *'more heavily'* weighted (half way between the horizontal state and the terrain responding state).
...
- 62 m (ancora ‘dimezzamento’, per risoluzione di calcolo della 'stima base' CALPUFF – ma non è esattamente lo stesso approccio!)<sup>*<sup/>
...
modello lagrangiano a particelle, tridimensionale, non stazionario, possibile utilizzazione per dispersione sostanze odorigene (peak-to-mean)
...
- valutare con attenzione condizioni di 'terreno complesso' e
- scegliere accuratamente input meteo

- considerare conseguenze dovute a condizioni di calma di vento 

- inserire sempre quota base di sorgente nel caso ‘regulatory ELEV' (non influente nel caso ‘non-regulatory FLAT’)

- definire attentamente parametri superficiali roughness, bowen ratio, albedo (*cfr. 'AERMOD implementation guide', p. 3.1.2*)

- leggere bene i manuali (tutti!), anche se non sempre chiari ed esaustivi 

- valutare ‘trade-off’ accuratezza/precisione dei risultati e tempi complessivi di elaborazione dello studio 
